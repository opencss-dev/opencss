...
let DOM = (el: string, all: boolean = true) => {
  try {
...
let strcopy = async (text: string): Promise<void> => {
  try {
...
let attrib = (el: string, attrib: any, set?: any) => {
  // Element selection
  let dom: any = select(el)
...
let set_attrib = (el: string, attrib: any, set?: any) => {
  // Element selection
  let dom: any = select(el)
...
let get_attrib = (el: string, attrib: any) => {
  // Element selection
  let dom: any = select(el)
 ...
 * */
let classlist = (el: string, tokens: string[]) => {
  let dom: any = select(el);
...
let select_id = (el: string): HTMLElement | null => {
  try {
    // Remove white space around the element
    if (typeof (el) == 'string' && el != null) el = el.trim()

    return document.getElementById(el)
  } catch (select_id_ERR: any) {
    // Debug select_id()
...
let drop_all = (list: string[]) => {
  list.forEach(el => {
    // Element selection from list
    const dom: any = select(el)
...
let drop = (token: string) => {
  // Element selection
  const dom: any = select(token)
...
let echo = (text: any, el?: any, all = false) => {
  if (isset(el)) {
    if (all) {
      ...
      let list: any = select(el, all)

      // for each element of the list add/replace content
      list.forEach((el: { innerHTML: string }) => {
        el.innerHTML = `${text}`
      })
...
let set_innerTEXT = (el: any, text?: string) => {
  try {
...
let hide = (el: any) => {
  ...
  const list: string[] = ['block', 'inline-block', 'flex', 'inline-flex']
...
let popUp = function (window = { title: ``, favicon: ``, content: ``, html: ``, href: ``, titlebar: true, rounded: true, shadow: true, classlist: [], style: ``, border: true, overlay: false, height: ``, width: ``, top: ``, right: ``, bottom: ``, left: ``, index: 8, timeout: 0 }) {
  ...
  WINDOW.addEventListener('mousemove', (e: any) => {
    let focus:any = select('.js_focus')
    let drag: any = get_attrib(`#${focus.id}`, 'aria-grabbed')
 ...
 * Cookies manage
 */
let set_cookie = (cookieName: any, cookieValue: any, cookieExpiry?: any) => {
  try {
...
let unset_cookie = (cookieName: any, cookieExpiry?: any) => {
  try {
    if (isset(cookieExpiry)) {
